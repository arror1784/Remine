...
export default function ParentNotifications() {
  ...
  const notifications = useNotificationStore((state) => state.parentNotifications)
  const markParentAsRead = useNotificationStore((state) => state.markParentAsRead)
  const unreadCount = notifications.filter((n) => n.unread).length

  const openNotification = (index: number, to: string) => {
    markParentAsRead(index)
    navigate(to)
  }
