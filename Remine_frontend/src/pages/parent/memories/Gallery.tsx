  ...
  return (
    <Screen footer={<BottomTabBar role="parent" accentColor="#ff42ad" />}>
      ...
      <div className="flex items-center justify-between px-5 py-3.5">
        <h1 className="text-[22px] font-semibold text-[#1a1a1a]">추억 갤러리</h1>
      ...
      <div className="flex flex-col gap-4 px-5 pb-10">
        <Link
          ...
          className="relative flex items-center gap-3.5 overflow-hidden rounded-[20px] bg-[#1c1c1c] px-5 py-4"
        >
          <div aria-hidden className="absolute -right-5 -top-5 size-20 rounded-full bg-remine-pink opacity-15 blur-[12px]" />
          <div className="flex size-[52px] shrink-0 items-center justify-center rounded-2xl bg-[#fff7cc] text-xl">🧩</div>
        ...
        <div className="grid grid-cols-2 gap-3">
          {MEMORIES.map((m) => (
            <div key={m.title} className="relative overflow-hidden rounded-[20px] bg-[#f2f2ee]">
              ...
              <div className="flex flex-col gap-1 px-3 pb-3.5 pt-2.5">
                <p className="text-[14px] text-[#1a1a1a]">{m.title}</p>
                <p className="text-[13px] text-[#9a9c91]">{m.date}</p>
              </div>
              {m.quizzed && (
        ...
        <button
          type="button"
          className="flex h-14 items-center justify-center gap-1.5 rounded-2xl border border-dashed border-[#fff7cc] bg-white text-[17px] font-semibold text-[#1a1a1a]"
        >
