...
const SUMMARY = [
  { emoji: '🌙', label: '수면', value: '7시간 12분', percent: 90, barColor: COLORS.blue },
  { emoji: '🚶', label: '활동량', value: '4,280보', percent: 55, barColor: COLORS.orange },
  { emoji: '🌿', label: '외출', value: '1회', percent: 40, barColor: COLORS.orange },
  { emoji: '💬', label: '사회 활동', value: '연락 없음', percent: 10, barColor: COLORS.borderMuted },
]

const PATTERNS = [
  { emoji: '🌙', label: '수면', value: '7시간 12분', tag: '안정', tagBg: COLORS.highlightBlue, tagColor: COLORS.teal },
  { emoji: '🚶', label: '활동량', value: '4,280보', tag: '조금 적음', tagBg: COLORS.highlightOrange, tagColor: COLORS.gold },
  { emoji: '🌿', label: '외출', value: '1회', tag: '조금 적음', tagBg: COLORS.highlightOrange, tagColor: COLORS.gold },
  { emoji: '💬', label: '사회 활동', value: '연락 없음', tag: '오늘은 휴식', tagBg: COLORS.surface, tagColor: COLORS.muted },
]
    ...
    <Screen footer={<BottomTabBar role="parent" accentColor={COLORS.pink} />}>
      ...
      <div className="flex flex-col gap-7 px-5 pb-8">
        ...
        <div className="flex flex-col gap-4 rounded-[20px] border border-remine-border bg-white p-5">
          {SUMMARY.map((s, i) => (
            <div key={s.label} className={`flex gap-3.5 ${i < SUMMARY.length - 1 ? 'border-b border-remine-surfaceSoft2 pb-4' : ''}`}>
        ...
        <div className="flex flex-col">
          ...
          {PATTERNS.map((p, i) => (
            <div key={p.label} className={`flex items-center gap-3.5 py-[18px] ${i < PATTERNS.length - 1 ? 'border-b border-remine-border' : ''}`}>
